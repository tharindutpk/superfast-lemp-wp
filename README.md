# Superfast LEMP Configuration for WordPress
...
- [Superfast LEMP Configuration for WordPress](#superfast-lemp-configuration-for-wordpress)
  ...
  - [Essentials](#essentials)
    ...
    - [Grant superuser privileges to the newly created user](#grant-superuser-privileges-to-the-newly-created-user)
  ...
  - [Install Nginx and Configure](#install-nginx-and-configure)
    ...
    - [Configure nginx for optimum performance](#configure-nginx-for-optimum-performance)
  - [Install PHP](#install-php)
  - [Setup UFW Firewall](#setup-ufw-firewall)
    - [Install `UFW` Firewall](#install-ufw-firewall)
  ...
  - [HTTPS and HTTPS/2](#https-and-https2)
...
``` bash
sudo mkdir -p /var/www/wpsnappy.com/{public_html,cache,logs}
```

``` bash
sudo chown -R tharindu: /var/www/wpsnappy.com
```
...
sudo cp /etc/nginx/sites-available/default /etc/nginx/sites-available/wpsnappy.com
sudo ln -s /etc/nginx/sites-available/wpsnappy.com /etc/nginx/sites-enabled/
sudo nginx -t
sudo systemctl restart nginx
```
...
sudo mv /tmp/wordpress/* /var/www/wpsnappy.com/public_html
```
...
``` bash
sudo nano /etc/nginx/sites-available/wpsnappy.com

# Add 'fastcgi_cache_path /var/www/wpsnappy.com/cache levels=1:2 keys_zone=wpsnappy.com:100m inactive=60m;' before the server block
```
...
``` nginx
fastcgi_cache_path /var/www/wpsnappy.com/cache levels=1:2 keys_zone=wpsnappy.com:100m inactive=60m;
